...
function historyIsNull() {
    return id('').findOne(1) !== null;
}
...
function findErrorWindow() {
  ...
  function isMainPage() {
    return id('net.one97.paytm:id/homeLogo').findOne(1) !== null;
  }
...
function getPhoneNumber(store) {
    id('net.one97.paytm:id/grp_title').findOne(1).click();
    id('net.one97.paytm:id/ln_name_tv').findOne(1).click();
    
    var phoneNumber;

    while(true){
      var obj = textMatches(/^\d{10}$/).find(); //正則表達匹配電話
      if (obj.length > 0) {
          for (var i = 0; i < obj.length; i++) {
              var phoneNumber = obj[i].text();
            //   log("電話號碼:" + "：" + phoneNumber);//test
...
function run() {
    while (true) {
      ...
      if (phoneNumber === null) {
        ...
        if (store.phoneNumber !== phoneNumber) {
            throw 'code:-5';
          }
      }
    ...
    if (id('net.one97.paytm:id/acc_item2').exists()) {
        var b = id('net.one97.paytm:id/acc_item2').findOne(1);
        b.click();
    } else {
        console.log("Error");
    }
